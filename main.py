...
QUESTIONS = [
    ...
    "Ты покупаешь из-за скидки, хотя заранее не планировал?",
...
async def calc_result(callback: CallbackQuery):
    ...
    text = (
        f"{calc_text}\n\n"
        "И это не “расходы”.\n\n"
        "Это деньги, которые ты мог оставить у себя.\n\n"
...
async def free_guide(callback: CallbackQuery):
    await callback.message.answer(
        "🎁 Забирай бесплатно.\n\n"
        "Но сделай с собой честную договорённость: не просто открыть файл, а применить его.\n\n"
        "Когда что-то достаётся легко, его очень просто отложить и забыть. "
        "Поэтому задай этому гайду ценность сам(а): выдели время, пройди его внимательно "
        "и начни с одного действия уже сегодня.\n\n"
        "Как только ты начнёшь вытаскивать из него максимум, результат тебя удивит."
    )
