...
GUIDE_PRICE_RUB = int(os.getenv("GUIDE_PRICE_RUB", "199"))
...
QUESTIONS = [
    ...
    "Ты покупаешь, когда устал, скучно или хочешь себя порадовать?",
    "Ты фиксируешь, сколько денег НЕ потратил?",
]
...
async def value_message(callback: CallbackQuery):
    ...
    await callback.message.answer(
        text,
        reply_markup=kb([[(f"💰 Оплатить {GUIDE_PRICE_RUB} ₽", "pay_guide")]]),
    )
    await callback.answer()
