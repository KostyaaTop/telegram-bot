    ...
    if os.path.exists(GUIDE_FILE):
        await bot.send_document(
            ...
            caption=(
                ...
                "Начни с первого блока и сегодня же зафиксируй первую сумму, которую не потратил(а)."
...
async def value_message(callback: CallbackQuery):
    text = (
        ...
        '<a href="https://disk.yandex.ru/i/BxD5CUVeQ4ayMw">Оферта</a>'
    )
...
async def pay_guide(callback: CallbackQuery):
    # Оплата через Telegram Payments временно отключена.
    # Позже здесь можно будет открыть ссылку/виджет Робокассы.
    await callback.answer()
